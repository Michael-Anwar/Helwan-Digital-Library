...
class Ahsan_Risalat_Eilmia2 extends StatefulWidget {
  final int? value;
  final Map<String, dynamic>? serr;

  const Ahsan_Risalat_Eilmia2({super.key, this.value, this.serr});

  @override
  _Ahsan_Risalat_Eilmia2State createState() => _Ahsan_Risalat_Eilmia2State();
}

class _Ahsan_Risalat_Eilmia2State extends State<Ahsan_Risalat_Eilmia2> {
  ...
  Future<void> initializeState() async {
    await getdata();
    if (status == 3) {
      await step2ser6get(_token!).then((_) {
        print(filePath); // Print after _level_controller2 is set
      });
    }
  }

  Future<void> step2ser6get(String token) async {
    try {
  ...
  Future<void> st2ser4(String token) async {
    try {
      ...
      Response response = await dio.put(
        'http://10.0.2.2:5001/StepTwoSer6/$id/$id2',
        data: formData,
