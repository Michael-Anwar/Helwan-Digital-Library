...
class Almajalaat_Eilmia2 extends StatefulWidget {
  final int? value;
  final Map<String, dynamic>? serr;

  const Almajalaat_Eilmia2({super.key, this.value, this.serr});

  @override
  _Almajalaat_Eilmia2State createState() => _Almajalaat_Eilmia2State();
}

class _Almajalaat_Eilmia2State extends State<Almajalaat_Eilmia2> {
...
Map<int, bool?> iswordFromApi = {};
  ...
  Future<void> initializeState() async {
    await getdata();
    if (status == 3) {
      await step2ser4get(_token!).then((_) {
        print(filePath); // Print after _level_controller2 is set
      });
    }
  }
  Future<void> step2ser4get(String token) async {
    try {
  ...
  Future<void> st2ser4(String token) async {
    try {
      Dio dio = Dio();
      ...
      Response response = await dio.put(
        'http://10.0.2.2:5001/StepTwoSer4/$id/$id2',
        data: formData,
  ...
  Future<void> handlesub() async {
    ...
    for (var m = 0; m < filenum!; m++) {
      if (pdfFilePaths[m + 1] == null || pdfFilePaths[m + 1]!.isEmpty) {
        if (!errorShown) {
          String error = "يجب ادخال البحث ${m + 1} pdf";
          ErrorDialog.showErrorDialog(context, error);
          errorShown = true;
        } else {
          break;
            ...
            title: Row(
              ...
              children: [
                Expanded(
                  ...
                  child: Text(
                                'فحص الابحاث العلمية بغرض النشر فى المجلات العلمية',
