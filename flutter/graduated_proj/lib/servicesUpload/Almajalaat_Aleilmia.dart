...
class Almajalaat_Aleilmia extends StatefulWidget {
  final int? value;
  final Map<String, dynamic>? serr;

  const Almajalaat_Aleilmia({super.key, this.value, this.serr});

  @override
  _Almajalaat_AleilmiaState createState() => _Almajalaat_AleilmiaState();
}

class _Almajalaat_AleilmiaState extends State<Almajalaat_Aleilmia> {
  ...
  Future<void> ser4get(String token) async {
    try {
  ...
  Future<void> ser4send(String token) async {
    try {
      Dio dio = Dio();
  ...
  Future<void> ser4(String token) async {
    try {
      Dio dio = Dio();
      dio.options.headers['Authorization'] = 'Bearer $token';
      dio.options.headers['Content-Type'] = 'multipart/form-data';
