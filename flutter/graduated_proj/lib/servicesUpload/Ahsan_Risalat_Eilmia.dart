...
class Ahsan_Risalat_Eilmia extends StatefulWidget {
  final int? value;
  final Map<String, dynamic>? serr;

  const Ahsan_Risalat_Eilmia({super.key, this.value, this.serr});

  @override
  _Ahsan_Risalat_EilmiaState createState() => _Ahsan_Risalat_EilmiaState();
}

class _Ahsan_Risalat_EilmiaState extends State<Ahsan_Risalat_Eilmia> {
  ...
  Future<void> ser6get(String token) async {
    try {
  ...
  Future<void> ser6send(String token) async {
    try {
      Dio dio = Dio();
  ...
  Future<void> ser6(String token) async {
    try {
      Dio dio = Dio();
      dio.options.headers['Authorization'] = 'Bearer $token';
      dio.options.headers['Content-Type'] = 'multipart/form-data';
 ...
 // var request = http.MultipartRequest(
      //   'POST',
      //   Uri.parse('http://10.0.2.2:5001/payment'),
      // );
      ...
      // request.fields['service_id'] = widget.value.toString();
      // request.fields['files_numbers'] = numberOfResearches.toString();
